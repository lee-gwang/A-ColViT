...
class ImageWithFixedHint(Dataset):
    def __init__(self, root, hint_dirs, transform=None, return_name=False, gray_file_list_txt=''):
        ...
        #     else:
        #         return rgb_path
        # img_dir = glob.glob(os.path.join(self.img_dir, '*/*'))
        # img_dir = Parallel(n_jobs=-1, backend='threading')(delayed(verify)(path) for path in tqdm(img_dir, total=len(img_dir)))
        # np.save('./fuckyou.npy', img_dir)
        # # img_dir = np.load('./fuckyou.npy')
        # self.img_list = ['/'.join(file.split('/')[-2:]) for file in img_dir if file != '']
        # self.img_list = sorted(self.img_list, key=lambda x: x.split('/')[-1])
        # temp = [x.split('/')[-1].split('.')[0]+'.txt' for x in self.img_list]
        ...
        for hint_dir in self.hint_dirs:
            ...
            # # gwang add
            # self.hint_list = [file for file in os.listdir(hint_dir)
            #                   if file.endswith('.txt') and file in temp]
