...
def get_args():
    ...
    parser.add_argument('--val_hint_list', default=[1,5,10,50, 100], nargs='+')
            ...
            for i, count in enumerate(args.val_hint_list):
                ...
                if args.pred_dir is not None:
                    ...
                    for name, image in zip(names, images):
                        torchvision.utils.save_image(image.unsqueeze(0), osp.join(
                            img_save_dir, osp.splitext(name)[0] + '_orig.png'))
                pbar.update()
